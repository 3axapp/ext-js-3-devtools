...
const DEV_TOOLS_HIGHLIGHT_NODE_ID = '____extJS3DevToolsHighlight';
...
function addHighlightForElement(el: Node, color: RgbColor = COLORS.blue, overlayType?: string): HTMLElement | null {
  ...
  if (overlayType) {
    if (rect.width > MINIMAL_OVERLAY_CONTENT_SIZE.width && rect.height > MINIMAL_OVERLAY_CONTENT_SIZE.height) {
...
function positionOverlayContent(overlayContent: HTMLElement, dimensions: DOMRect, labelPosition: 'inside' | 'outside') {
...
function createOverlaySvgElement(type: string): Node {
  let icon: string;
  if (type === 'hydrated') {
    icon = HYDRATION_SVG;
