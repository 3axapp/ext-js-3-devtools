...
export function createLevelSerializedDescriptor(
  ...
  continuation: (instance: any, propName: string | number, level?: number, max?: number) => Descriptor,
): Descriptor {
...
function getLevelDescriptorValue(
  ...
  switch (type) {
    case PropType.Array:
      return value.map((_: any, idx: number) => continuation(value, idx, currentLevel + 1, level));
